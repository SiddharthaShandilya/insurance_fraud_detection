...
class RawDataValidation:
    ...
    def get_value_from_schema(self):
        ...
        """
        logging.info(
            "get_value_from_schema function from Raw_Data_Validation class is called"
        )
        try:
    ...
    def raw_file_validation(
        self, input_filename, LengthOfDateStampInFile, LengthOfTimeStampInFile
    ):
        """
        ...
        logging.info(
            "Running raw_file_name_validation function for validating the input filename"
        )
        try:
            if re.match(INPUT_FILE_NAME_REGEX, input_filename):
                split_raw_file_name_at_dot = re.split(".csv", input_filename)
                split_raw_file_name_at_underscore = re.split(
                    "_", split_raw_file_name_at_dot[0]
                )
                if len(split_raw_file_name_at_underscore[1]) == LengthOfDateStampInFile:
                    if (
                        len(split_raw_file_name_at_underscore[2])
                        == LengthOfTimeStampInFile
                    ):
                        logging.info(
                            "File name validated moving it to good data folder"
                        )
                        shutil.copy(src=remote_raw_file_name, dst=self.good_data_dir)
                        logging.info(
                            f"File name validated moved, \n updated list of files it in good data folder:{os.listdir(self.good_data_dir)} "
                        )
                        return 1
                    else:
                        logging.info(
                            "LengthOfTimeStampInFile not matched moving to bad data folder"
                        )
                        shutil.copy(src=remote_raw_file_name, dist=self.bad_data_dir)
                        return 0
                else:
                    logging.info(
                        "LengthOfDateStampInFile not matched moving to bad data folder"
                    )
                    shutil.copy(src=remote_raw_file_name, dist=self.bad_data_dir)
                    return 0
            else:
    ...
    def raw_file_column_length_validation(self, NumberofColumns):
        ...
        """
        logging.info(
            "raw_file_column_length_validation from data vlidation class function called"
        )
        try:
            for files in os.listdir(self.good_data_dir):
                ...
                if len(raw_file_data.columns) == NumberofColumns:
                    logging.info(
                        f"{self.good_data_dir}/{files} has valid column length"
                    )
                    return 1
                else:
                    logging.info(
                        f"{self.good_data_dir}/{files} has invalid column length moving file to bad data folder"
                    )
                    shutil.move(
                        src=f"{self.good_data_dir}/{files}", dst=self.bad_data_dir
                    )
                    return 0
            ...
            for file in os.listdir(self.good_data_dir):
                ...
                for i in range(len(extracted_data_columns)):
                    if colName_list[i] != extracted_data_columns[i]:
                        ...
                        return 0
            logging.info(
                f"raw_file_column_name is validated for file {self.good_data_dir}/{file}"
            )
            return 1
        except Exception as e:
            logging.info(e)
            ...
            for file in os.listdir(self.good_data_dir):
                ...
                for columns in csv:
                    if (len(csv[columns]) - csv[columns].count()) == len(csv[columns]):
                        count += 1
                        shutil.move(
                            os.path.join(self.good_data_dir, file), self.bad_data_dir
                        )
                        logging.info(
                            f"Invalid Column for the file!! File moved to {self.bad_data_dir}"
                        )
                        return 0
                if count == 0:
                    logging.info(
                        f"valid Column for the file!! File moved to {self.good_data_dir}"
                    )
                    csv.to_csv(
                        os.path.join(self.good_data_dir, file), index=None, header=True
                    )
                    return 1
        except OSError:
    ...
    def move_bad_files_to_archive_bad(self):
        ...
        try:
            ...
            if os.path.isdir(source):
                ...
                dest = (
                    self.training_archive_bad_data_dir_path
                    + "/BadData_"
                    + str(date)
                    + "_"
                    + str(time)
